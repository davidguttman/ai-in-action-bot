...
async function hasTopicBeenCovered(queryTopic) {
  try {
    ...
    const topicsForLLM = pastTalks
      .map((talk) => {
        const dateStr = talk.scheduledDate
          ? talk.scheduledDate.toISOString().split('T')[0]
          : 'unknown date'
        return `${talk.topic} (${dateStr})`
      })
      .join('\n')

    const systemMessage = `You are an assistant helping determine if a specific topic has been covered in past talks.
Today's date is ${today}. The talks listed below are all past talks that occurred before today or are marked as completed.
Given this list of past talk topics with their dates and a query topic, determine if the query topic is similar to any past topic.
Respond with ONLY 'yes' if the topic has been covered or 'no' if it hasn't.`

    const prompt = `Past talk topics with dates:\n${topicsForLLM}\n\nQuery topic: ${queryTopic}`
...
Today's date is ${today}. The talks listed below are all past talks that occurred before today or are marked as completed.
Given this list of past talks with their details and a query topic, return the IDs of talks that are related to the query topic.
