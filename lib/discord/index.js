    ...
    if (
      ...
      try {
        ...
        // --- Handle 'other' or Unrecognized Intent ---
          ...
          // Check for source code / GitHub related queries
          ...
          const messageContainsSourceCodeQuery = sourceCodeKeywords.some(
            (keyword) =>
              userMessageContent.toLowerCase().includes(keyword.toLowerCase()),
          )

          if (messageContainsSourceCodeQuery) {
            console.log(
              'Source code query detected, providing GitHub repository link',
            )
            try {
              await message.reply(
              ...
              console.error('Failed to send source code reply:', e)
            }
          } else {
            // Send a generic help message if intent is 'other' or unclear
            try {
              await message.reply(
                "How can I help? You can ask me to 'sign up', 'view schedule', 'cancel talk', ask about past talks, or ask for the 'source code'!",
              )
            } catch (e) {
              console.error(e)
            }
