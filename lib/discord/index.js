  ...
  client.on(Events.MessageCreate, async (message) => {
    ...
    if (
      ...
      // --- State: awaiting_topic_for_others ---
        ...
        try {
          const topicCheckSystemMessage =
            'You are validating if a message contains a reasonable presentation topic. Respond with "valid" if it looks like a topic, or "invalid" if it\'s too short, nonsensical, or not topic-like.'
          const intentResponse = await completion({
            systemMessage: topicCheckSystemMessage,
            prompt: userMessage,
          })

          if (intentResponse?.trim().toLowerCase() !== 'valid') {
            try {
              await message.reply(
