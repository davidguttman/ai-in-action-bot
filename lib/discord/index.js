  ...
  client.on(Events.MessageCreate, async (message) => {
    ...
    if (
      ...
      try {
        ...
        // --- Handle 'repo' Intent ---
        else if (detectedIntent === 'repo') {
          console.log('Repository request intent detected.')
          try {
            await message.reply(
              '<https://github.com/davidguttman/ai-in-action-bot|https://github.com/davidguttman/ai-in-action-bot>',
            )
          } catch (replyError) {
            console.error('Failed to send repository reply:', replyError)
          }
        }
        // --- Handle 'other' or Unrecognized Intent ---
          ...
          try {
            await message.reply(
              "How can I help? You can ask me to 'sign up', 'schedule someone else', 'view schedule', 'cancel talk', ask about past talks, get the zoom link, or ask for my GitHub repo to make me better!",
            )
          } catch (e) {
            console.error(e)
          }
