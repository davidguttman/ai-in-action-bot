  ...
  client.on(Events.MessageCreate, async (message) => {
    ...
    if (message.channel.isThread() && signupInfo && message.author.id === signupInfo.userId) {
      ...
      if (signupInfo.state === 'awaiting_topic') {
        ...
        const topicCheckSystemMessage = "You are an assistant helping determine if a user's message is a presentation topic. Respond with ONLY 'topic' if it seems like a plausible topic, or 'clarify' if it's conversational filler, a question, or clearly not a topic."
          ...
          if (selectedDateObject) {
            ...
            try { // Inner try for booking + confirmation reply
              ...
              if (bookingResult) {
                ...
                console.warn(`scheduleSpeaker returned null/falsy for thread ${threadId}, possibly due to handled conflict.`)
                ...
                if (!newAvailableDates || newAvailableDates.length === 0) {
                  ...
                  } catch (replyError) { console.error('Failed to send conflict/no new dates reply:', replyError) }
                  delete activeSignups[threadId]
                } else {
                  ...
                  try {
                    await message.reply(`It seems there was an issue booking that exact slot. Here are the updated available dates:\n${formattedNewDates}\nWhich of these works?`)
                  } catch (replyError) { console.error('Failed to send conflict/re-proposal reply:', replyError) }
                  // State remains 'awaiting_date_selection'
                }
              }
            } catch (bookingError) {
              // CONFLICT HANDLING (Specifically check for duplicate key error code 11000)
              if (bookingError.code === 11000 && bookingError.keyPattern && bookingError.keyPattern.scheduledDate) {
    ...
    if (!message.channel.isThread() && (trimmedContent.startsWith(mentionPrefix1) || trimmedContent.startsWith(mentionPrefix2))) {
      console.log(`Bot mentioned at start by ${message.author.tag} in channel ${message.channel.id}`)
      ...
      try { // Outer try for intent detection + handling
        ...
        if (detectedIntent === 'sign_up') {
          if (!message.channel.threads) {
             ...
             } catch (replyError) { console.error('Failed to send no thread support reply:', replyError) }
             return
          }
        ...
        // --- Handle 'cancel_talk' Intent ---
            ...
            try { // Inner try for cancelling talk + replying
              ...
              if (cancelledTalk) {
                ...
                } catch (replyError) { console.error('Failed to send cancel confirmation reply:', replyError) }
              } else {
                try { // Wrap reply
                  await message.reply("You don't seem to have an upcoming talk scheduled that I can cancel.")
                } catch (replyError) { console.error('Failed to send no talk to cancel reply:', replyError) }
              }
            } catch (error) {
              ...
              } catch (replyError) { console.error('Failed to send cancel error reply:', replyError) }
            }
        }
