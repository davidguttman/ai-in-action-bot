...
`
            ...
            const formatPaginatedTalks = (result, requestMeta) => {
              ...
              const lines = [header, `Showing ${talks.length} • Page ${page} of ${totalPages} • ${direction === 'asc' ? 'Oldest first' : 'Newest first'}`]
            ...
            if (talkRequest.type === 'existence_check' && talkRequest.topic) {
              ...
              if (relatedTalks.length) {
                ...
                return message.reply(`Yes, there have been talks about ${talkRequest.topic}:\n${items}`)
              }
              return message.reply(`No, I don't see any previous talks about ${talkRequest.topic}.`)
            }
            ...
            if (talkRequest.type === 'list_topic' && talkRequest.topic) {
              ...
              if (relatedTalks.length) {
                const items = relatedTalks.map((t) => formatTalkLine(t)).join('\n')
                return message.reply(`Here are talks related to ${talkRequest.topic}:\n${items}`)
              }
              ...
              return message.reply(`I couldn't find any talks related to ${talkRequest.topic}.`)
            }
            ...
            if (talkRequest.type === 'list_speaker' && talkRequest.speaker) {
              ...
              if (!listResult.talks.length)
                return message.reply(`I couldn't find any past talks for ${talkRequest.speaker}.`)
            ...
            const listResult = await talkHistory.listPastTalks({
              ...
              direction: talkRequest.direction,
              speaker: talkRequest.speaker,
              topic: talkRequest.type === 'list_general' ? talkRequest.topic : null,
            })
