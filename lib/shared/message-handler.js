...
const { getRecentMergeChanges, formatMergeChanges } = require('../git/recentChanges')
  ...
  return async function handleMessage(message) {
    ...
    if (message.channel.isThread() && signupInfo && message.author.id === signupInfo.userId) {
      ...
      if (signupInfo.state === 'awaiting_reschedule_date_selection') {
        ...
        const proposedDates = signupInfo.proposedDates || []
        if (!proposedDates.length) {
          await message.reply("Sorry, something went wrong and I lost the proposed dates. Please try again.")
          delete activeSignups[message.channel.id]
          return
        }
        ...
        if (selectedIndex >= 0 && selectedIndex < proposedDates.length) {
          ...
          try {
            const result = await rescheduleSpeaker(message.author.id, selectedDate)
            if (result) {
              const d = selectedDate.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
              await message.reply(`All set! I moved your talk "${signupInfo.currentTopic}" to ${d}.`)
              delete activeSignups[message.channel.id]
              return
            }
            // null => date booked
            ...
            await message.reply(`That date is taken. Here are updated options:\n${formatted}\nWhich works?`)
            return
          } catch (err) {
            await message.reply('Sorry, I ran into a problem rescheduling. Please try again later.')
            delete activeSignups[message.channel.id]
            return
          }
        }
        const formattedOriginal = formatDatesForDisplay(proposedDates)
        await message.reply(`Sorry, I didn't quite catch that. Please reply with 1, 2, or 3:\n${formattedOriginal}`)
        return
      }
      ...
      if (signupInfo.state === 'awaiting_target_user_for_cancel') {
        ...
        if (!match) {
          await message.reply("Please mention the person whose talk you'd like me to cancel (e.g., @username).")
          return
        }
        ...
        try {
          ...
          if (cancelled) {
            ...
            await message.reply(`Okay, I have cancelled @${targetUser.username}'s talk "${cancelled.topic}" scheduled for ${d}.`)
          } else {
            await message.reply(`I don't see an upcoming talk for @${targetUser.username} to cancel.`)
          }
        } catch (e) {
          await message.reply("I couldn't find that user. Please make sure you're mentioning a valid user.")
        } finally {
          delete activeSignups[message.channel.id]
        }
        return
      ...
      if (signupInfo.state === 'awaiting_target_user_for_reschedule') {
        ...
        const match = userMessage.match(/<@!?([\w-]+)>/)
        if (!match) {
          await message.reply("Please mention the person whose talk you'd like me to reschedule (e.g., @username).")
          return
        }
        const targetUserId = match[1]
        try {
          ...
          if (!current) {
            await message.reply(`I don't see an upcoming talk for @${targetUser.username} to reschedule.`)
            delete activeSignups[message.channel.id]
            return
          }
          const availableDates = await findAvailableFridays(3)
          if (!availableDates.length) {
            await message.reply("Sorry, I couldn't find any available slots to move their talk.")
            delete activeSignups[message.channel.id]
            return
          }
          ...
          const cd = current.scheduledDate.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
          await message.reply(`@${targetUser.username} is currently scheduled on ${cd} for "${current.topic}". Here are the next available Fridays:\n${formatted}\nWhich date works best? (Reply with 1, 2, or 3)`) 
        } catch (e) {
          await message.reply("I couldn't find that user. Please make sure you're mentioning a valid user.")
          delete activeSignups[message.channel.id]
        }
      ...
      if (signupInfo.state === 'awaiting_reschedule_date_selection_for_others') {
        const userReply = message.content.trim()
        const { proposedDates, targetUserId, targetUsername, currentTopic } = signupInfo
        if (!proposedDates || proposedDates.length === 0) {
          await message.reply("Sorry, I lost the proposed dates. Please start the reschedule process again.")
          delete activeSignups[message.channel.id]
          return
        }
        let selectedIndex = -1
        ...
        if (selectedIndex >= 0 && selectedIndex < proposedDates.length) {
          const selectedDateObject = proposedDates[selectedIndex]
          try {
            const result = await rescheduleSpeaker(targetUserId, selectedDateObject)
            if (result) {
              const d = selectedDateObject.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
              await message.reply(`Done! I moved @${targetUsername}'s talk "${currentTopic}" to ${d}.`)
              delete activeSignups[message.channel.id]
              return
            }
            // date booked
            const newDates = await findAvailableFridays(3)
            if (!newDates.length) {
              await message.reply('Oops! That date was just booked and I could not find other options right now. Please try later.')
              delete activeSignups[message.channel.id]
              return
            }
            signupInfo.proposedDates = newDates
            const formatted = formatDatesForDisplay(newDates)
            await message.reply(`That date is taken. Updated options:\n${formatted}\nWhich works?`)
            return
          } catch (err) {
            await message.reply('Sorry, I ran into a problem rescheduling. Please try again later.')
            delete activeSignups[message.channel.id]
    ...
    const isSignupThread = message.channel.isThread() && !!activeSignups[message.channel.id]
    const existingTalkHistoryContext = talkHistoryContexts[message.channel.id]
    const shouldHandleWithoutMention = message.channel.isThread() && !!existingTalkHistoryContext
    if ((mentionStarts && !isSignupThread) || shouldHandleWithoutMention) {
      ...
      try {
        ...
        if (detectedIntent === 'reschedule_talk') {
          if (!message.channel.threads) {
            await message.reply("Sorry, I can't create rescheduling threads in this channel.")
            return
          }
          const current = await getUserUpcomingTalk(message.author.id)
          if (!current) {
            return message.reply("You don't seem to have an upcoming talk to reschedule.")
          }
          const thread = await message.startThread({
          ...
          const availableDates = await findAvailableFridays(3)
          if (!availableDates || availableDates.length === 0) {
            await thread.send("Sorry, I couldn't find any available slots to move your talk. Please check back later or contact an admin.")
            return
          }
          ...
          const formatted = formatDatesForDisplay(availableDates)
          const currentDateString = current.scheduledDate.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
          await thread.send(`You are currently scheduled to speak on ${currentDateString} about "${current.topic}". Here are the next available Fridays to reschedule to:\n${formatted}\nWhich date works best? (Reply with 1, 2, or 3)`)          
          return
        }
          ...
          try {
            ...
            if (!message.channel.isThread() && message.channel.threads) {
              try {
                ...
                previousContext = talkHistoryContexts[contextKey] || previousContext
              } catch (err) {
                console.error('Failed to start talk history thread:', err)
                await message.reply('Sorry, I ran into an issue creating a thread for that. Please try again later.')
                return
              }
            }
            ...
            const isReplyingInThread = !!historyThread || message.channel.isThread()
...
`
            ...
            const continuationHintRegex = /(\bpage\b|\bnext\b|\bprev\b|\bprevious\b|\bback\b|\banother\b|\bmore\b)/

            const explicitTopic = typeof parsedQuery.topic === 'string' && parsedQuery.topic.trim()
            const explicitSpeaker = typeof parsedQuery.speaker === 'string' && parsedQuery.speaker.trim()
            ...
            talkRequest.page = normalisePositiveInt(talkRequest.page, defaults.page)
            if (!['asc', 'desc'].includes(talkRequest.direction)) talkRequest.direction = defaults.direction
            if (!['month', 'year'].includes(talkRequest.groupBy)) talkRequest.groupBy = defaults.groupBy
            if (talkRequest.topic) talkRequest.topic = talkRequest.topic.trim()
            if (talkRequest.speaker) talkRequest.speaker = talkRequest.speaker.trim()
            ...
            if (isContinuation && previousContext) {
              if (!explicitType || explicitType === 'list_general') talkRequest.type = previousContext.type || talkRequest.type
              if (!explicitTopic && previousContext.topic) talkRequest.topic = previousContext.topic
              if (!explicitSpeaker && previousContext.speaker) talkRequest.speaker = previousContext.speaker
              if (!explicitStart && previousContext.startDate) talkRequest.startDate = previousContext.startDate
              if (!explicitEnd && previousContext.endDate) talkRequest.endDate = previousContext.endDate
              if (!explicitGroup && previousContext.groupBy) talkRequest.groupBy = previousContext.groupBy
              if (!explicitDirection && previousContext.direction) talkRequest.direction = previousContext.direction
              if (!explicitPageSize && previousContext.pageSize) talkRequest.pageSize = previousContext.pageSize

              if (!explicitPage) {
                const previousPage = previousContext.page || 1
                if (wantsNext) talkRequest.page = previousPage + 1
                else if (wantsPrevious) talkRequest.page = Math.max(1, previousPage - 1)
            ...
            const formatTalkLine = (talk) => {
              const date = talk.scheduledDate ? new Date(talk.scheduledDate) : null
              const formattedDate = date
                ...
                : 'Unknown date'
              const speaker = talk.discordUsername ? `@${talk.discordUsername}` : 'Unknown speaker'
              return `- ${formattedDate}: ${speaker} — "${talk.topic}"`
            }

            const groupTalks = (talks, groupBy) => {
              if (!groupBy) return null
              const normalised = groupBy === 'year' ? 'year' : groupBy === 'month' ? 'month' : null
            ...
            const formatPaginatedTalks = (result, requestMeta) => {
              const { talks, page, pageSize, total, hasMore, startDate, endDate, direction } = result
              if (!talks.length) return "I couldn't find any past talks for that request."

              const totalPages = total > 0 ? Math.ceil(total / pageSize) : 1
              const descriptorParts = []
              if (requestMeta.topic) descriptorParts.push(`about ${requestMeta.topic}`)
              if (requestMeta.speaker) descriptorParts.push(`by ${requestMeta.speaker}`)
              if (startDate || endDate) {
                const pieces = []
                if (startDate) pieces.push(`from ${ensureDateString(startDate)}`)
            ...
            if (talkRequest.type === 'existence_check' && talkRequest.topic) {
              const relatedTalks = await talkHistory.findRelatedTalks(talkRequest.topic)
              if (relatedTalks.length) {
                const items = relatedTalks.map((t) => formatTalkLine(t)).join('\n')
                updateTalkHistoryContext({
                  request: { ...talkRequest, page: 1, pageSize: relatedTalks.length },
                  result: { page: 1, pageSize: relatedTalks.length, talks: relatedTalks },
                })
                return sendReply(`Yes, there have been talks about ${talkRequest.topic}:\n${items}`)
              }
              return sendReply(`No, I don't see any previous talks about ${talkRequest.topic}.`)
            }
            ...
            if (talkRequest.type === 'list_topic' && talkRequest.topic) {
              ...
              if (relatedTalks.length) {
                ...
                return sendReply(`Here are talks related to ${talkRequest.topic}:\n${items}`)
              }
              ...
              if (fallbackList.talks.length) {
                const formatted = formatPaginatedTalks(fallbackList, talkRequest)
                updateTalkHistoryContext({ request: talkRequest, result: fallbackList })
                return sendReply(formatted)
              }
              return sendReply(`I couldn't find any talks related to ${talkRequest.topic}.`)
            }
            ...
            if (talkRequest.type === 'list_speaker' && talkRequest.speaker) {
              ...
              if (!listResult.talks.length)
                return sendReply(`I couldn't find any past talks for ${talkRequest.speaker}.`)
              const formatted = formatPaginatedTalks(listResult, talkRequest)
              updateTalkHistoryContext({ request: talkRequest, result: listResult })
              return sendReply(formatted)
            }
            ...
            if (isContinuation && wantsNext && !listResult.talks.length) {
              return sendReply("You're already seeing the latest page of past talks.")
            }
            ...
            return sendReply('Sorry, I encountered an error while searching for past talks.')
          }
        }
        ...
        if (detectedIntent === 'schedule_for_others') {
          ...
          if (mentionedUser) {
            const targetUsername = mentionedUser.globalName || mentionedUser.username || mentionedUser.tag
            activeSignups[thread.id] = {
              userId: message.author.id,
            ...
            await thread.send(`Hi ${message.author}, I'll help you schedule ${targetUsername} to speak! What topic would you like them to present on?`)
            return
          }
        ...
        if (detectedIntent === 'cancel_talk_for_others') {
          if (!message.channel.threads) {
            await message.reply("Sorry, I can't create cancellation threads in this channel.")
            return
          }
          const thread = await message.startThread({
          ...
          if (mentionedUser) {
            ...
            try {
              ...
              if (cancelled) {
                ...
                await thread.send(`Okay, I have cancelled @${targetUsername}'s talk "${cancelled.topic}" scheduled for ${d}.`)
              } else {
                await thread.send(`I don't see an upcoming talk for @${targetUsername} to cancel.`)
              }
            } catch (e) {
              await thread.send("I couldn't find that user. Please make sure you're mentioning a valid user.")
            }
            return
          }
          activeSignups[thread.id] = { userId: message.author.id, state: 'awaiting_target_user_for_cancel', lastUpdated: Date.now() }
          await thread.send(`Okay! Please mention the person whose upcoming talk you'd like me to cancel (e.g., @username).`)
          return
        }
        ...
        if (detectedIntent === 'reschedule_talk_for_others') {
          ...
          if (mentionedUser) {
            ...
            try {
              ...
              if (!current) {
                await thread.send(`I don't see an upcoming talk for @${targetUsername} to reschedule.`)
                return
              }
              const availableDates = await findAvailableFridays(3)
              if (!availableDates.length) {
                await thread.send("Sorry, I couldn't find any available slots to move their talk.")
                return
              }
              const formatted = formatDatesForDisplay(availableDates)
          ...
          activeSignups[thread.id] = { userId: message.author.id, state: 'awaiting_target_user_for_reschedule', lastUpdated: Date.now() }
          await thread.send(`Great! Please mention the person whose talk you'd like me to reschedule (e.g., @username).`)
          return
        }

        if (detectedIntent === 'repo') {
          return message.reply('https://github.com/davidguttman/ai-in-action-bot')
        }
        ...
        if (detectedIntent === 'recent_changes') {
          try {
            ...
            return message.reply("Sorry, I couldn't pull the recent history right now.")
          }
        }

        // other
        return message.reply("How can I help? You can ask me to 'sign up', 'schedule someone else', 'view schedule', 'reschedule talk', 'cancel talk', 'reschedule someone else', 'cancel someone else', ask about past talks, get the zoom link, or ask for my GitHub repo!")
      } catch (e) {
